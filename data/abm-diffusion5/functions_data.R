...
get_data_replication <- function(results){
  ...
  df_time_series <- df_time_series_rep_filled |> 
    ...
    summarise(
      ...
      D_share_mean = mean(D, na.rm = TRUE),
      D_share_sd   = if (n() > 1) sd(D, na.rm = TRUE) else 0,
      replications = replications,
      .groups = "drop"
    )
  
  # 4) df_final_summary
  # Assume num_of_types = 4 and types are coded 1 (S), 2 (I), 3 (R), 4(D)
  df_final_summary <- df_final_worlds_rep |>
    count(rep, type) |>
    complete(rep, type = 1:4, fill = list(n = 0)) |>
    group_by(rep) |>
